-- Full Join: No need common column, nullish value can be find
-- Natural Join: One common column need, no nullish value, no on keyword for join. At first cross join performed then natural join performed based on common column data. If data are not matched then it filter out from the table.
-- Cross Join: no nullish value, no on keyword for join
...
select * from employees e full join departments on e.department_id=departments.department_id ;
