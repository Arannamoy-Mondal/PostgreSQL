...
-- 2. **Group By Department with Average Salary**
    select d.department_name,avg(employees.salary) from employees join departments as d on
        employees.department_id = d.department_id group by d.department_name;

-- 3. **Count Employees in Each Department**
    select departments.department_name , count(employees.employee_id) from employees join departments using (department_id) group by departments.department_name ;
-- 4. **Find the Department name with the Highest Average Salary**
    select d.department_name,avg(employees.salary) as avg_salary from employees join departments as d on
        employees.department_id = d.department_id group by d.department_name order by avg_salary desc limit 1;
